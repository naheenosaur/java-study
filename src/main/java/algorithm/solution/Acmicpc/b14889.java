...
public class b14889 {
    ...
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            List<Integer> me = nextPermutation(temp);
            if (me.isEmpty()) {
                return;
            }
            for (int i = 0; i < 5; i++) {
    ...
    static List<Integer> nextPermutation(List<Integer> now) {
        ...
        if (hasNext) {
            ...
            int nextIndex = index;
            for (int i = index + 1; i < now.size() - 1; i++) {
                if (now.get(index) < now.get(i)) {
                    nextIndex = i;
                }
            }

            next.add(now.get(nextIndex));

            for (int i = now.size() - 1; i > nextIndex; i--) {
                next.add(now.get(i));
            }
