...
public class b9019 {
    static Integer startNum;
    static Integer destNum;
    ...
    public static void dslr() {
        Map<Integer, String> traveled = new HashMap<>();
        List<Integer> bst = new ArrayList<>();
        traveled.put(startNum, "");
        bst.add(startNum);
        while (true) {
            Integer num = bst.remove(0);
            Integer d = (2 * num) % 10000;
            Integer s = (num == 0 ? 9999 : num - 1);
            Integer l = (num % 1000) * 10 + (num / 1000);
            Integer r = (num % 10) * 1000 + (num / 10);

            if (!traveled.containsKey(d)) {
                bst.add(d);
                traveled.put(d, traveled.get(num).concat("D"));
            }
            if (!traveled.containsKey(s)) {
                bst.add(s);
                traveled.put(s, traveled.get(num).concat("S"));
            }
            if (!traveled.containsKey(l)) {
                bst.add(l);
                traveled.put(l, traveled.get(num).concat("L"));
            }
            if (!traveled.containsKey(r)) {
                bst.add(r);
                traveled.put(r, traveled.get(num).concat("R"));
            }

            if (bst.contains(destNum)) {
                System.out.println(traveled.get(destNum));
                return;
            }
        }
    }
