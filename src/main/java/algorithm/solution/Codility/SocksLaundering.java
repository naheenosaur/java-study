...
Each sock has its own color. Bob wants to take as many pairs of clean socks as possible (both socks in the pair should be of the same color).
...
He wants to pick socks for laundering in such a way that after washing he will have a maximal number of clean, same-colored pairs of socks.
It is possible that some socks cannot be paired with any other sock, because Bob may have lost some socks over the years.
...
If Bob's washing machine can clean at most K = 2 socks, then he can take a maximum of three pairs of clean socks.
 He can wash one red sock and one green sock, numbered 1 and 2 respectively. Then he will have two pairs of red socks and one pair of green socks.
...
public class SocksLaundering {
    public static void main(String[] args) {
        ...
        //        int K = 4;
        //        int[] C = {1, 2, 5, 3, 4, 3, 1};
        //        int[] D = {1, 4, 3, 2, 2, 7, 3,4}; // 1, 1, 3, 3, 2, (2), 4, (4), 3, 3
    ...
    public static int solution(int K, int[] C, int[] D) {
        ...
        return sum + Math.min(possible, Math.round(K / 2));
    }
}
